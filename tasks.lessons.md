...
- **Port the invariant, not the infrastructure.** The one-loop cache tables existed only because Realtime broadcasts table changes; on a polling foundation they're dead weight — truth-is-the-view came through, the caches didn't. Ask what each piece was FOR before porting it. (CODE-E, 2026-07-02.)
- **Role-lens passes catch what unit tests don't.** The events log leaked ledger amounts into the journey for budgets-gated users — every module gate needs checking at every surface that renders the data, not just the module's own endpoint. (CODE-E.)
- **One review pattern beats four import flows.** Every intake path (file, card, type-ahead, sentence) converges on Bundle → plan-diff → the same preview grid → commit. New paths cost a parser, not a UI. And plan/commit as separate server steps gives idempotent re-import almost for free — the diff IS the dedupe. (CODE-C, 2026-07-01.)
- **AI parsing behind a seam, with a keyless fallback.** NL capture works without ANTHROPIC_API_KEY via a labeled heuristic; Claude slots in when the key lands. The founder can dogfood the flow before the credential exists. (CODE-C.)
